...
1. آشنایی با GitHub Actions و اضافه کردن CI/CD Pipeline به ریپازیتوری:
    ...
    1.
         ...
         مراحل Pipeline را به ترتیب تعریف می‌کنیم. در ابتدا مشخص می‌کنیم که pipeline روی آخرین ورژن ubuntu اجرا شود: (می‌توان آن را تغییر داد, اگر برای این میخواید تغییر بدید که file path
        ...
        1. در این مرحله مشخص می‌کنیم که CI از agent ای به نام `actions/checkout@v2` استفاده کند: (برای مطالعه بیشتر [این](https://www.edwardthomson.com/blog/) لینک را بخوانید.)
            (در واقع با این خط گیت هاب میفهمد که باید از  فایل‌هایی شبیه  فایل شما که از قبل درسته  شدن استفاده کند)
