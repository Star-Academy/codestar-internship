...
برای ذخیره ی مقدار جدید کوکی، تنها کافی است `document.cookie` را مقدار دهی کنیم. توجه داشته باشید که مقادیر `document.cookie`  تنها مختص به همان صفحه هستند و در صورتی که به صفحه ی دیگری از وب برویم، دیگر نمی توانند مورد استفاده قرار بگیرند. 
<div dir="ltr">
  ...
  مقدار `expires`  مشخص می کند که کوکی ها چه زمانی منقضی شوند. در صورتی که بخواهیم مقدار کوکی های یک صفحه دیگر را عوض کنیم، از `path` استفاده می کنیم. در مثال بالا، در صورت خواندن مقدار `document.cookie` در `domain.com/link.html`، مقدار <span dir="ltr"> `username=codestar;password=1234;` </span> را دریافت می کنیم. برای ست کردن کوکی ها در همین صفحه، مقدار `/=path` بگذارید. 
توجه داشته باشید که در کوکی ها تنها می توانیم مقادیر `string` ای بخوانیم و بنویسیم.
  ...
  مشکلی که با کوکی ها وجود داشت، مقدار حافظه ی کم آن ها (حداکثر 4 کیلوبایت) بود. در نتیجه   `session storage` و `local storage` معرفی شدند. بر خلاف کوکی ها، در این دو نوع از حافظه از ذخیره سازی `key, value` استفاده می شود. (مانند دیکشنری ها).
  
  ### Session storage: 
  
  `Session storage` نوع دیگری از حافظه در وب است. زمانی که پنجره و تب بسته می شود، اطلاعاتی هم که توسط `session storage`  ذخیره شده هم پاک می شود. بنابراین وقتی که تب جدیدی باز می شود، یک `session` جدید آغاز می شود و اطلاعات ذخیره شده در `session storage` تب های دیگر، اینجا قابل استفاده نیست.
...
ست کردن و گرفتن دیتا در `local storage` دقیقا مشابه `session storage` می باشد. 
  ...
  این دستور مقدار `key` , `value` مورد نظر را از حافظه لوکال پاک می کند. 
...
برای خواندن تفاوت حافظه ی کوکی و لوکال می توانید به لینک زیر مراجعه کنید:
...
مبحث `Promise` بسیار گسترده ست. برای اطلاعات بیشتر می تونید از لینک زیر استفاده کنید:
...
<span dir="ltr">
 ...
 ### async, await:
 در صورتی که پشت نام یک تابع `async` به کار رود، آن تابع یک `Promise` به عنوان خروجی بر می گرداند. `async`و `await` یک راه تمیز تر و ساده تر برای استفاده از `Promise` ها هستند. 
...
در ادامه با `fetch`، کاربرد های بیشتری را می بینیم. 
...
در حالت کلی، ورودی های تابع `url` ،`fetch`  و `option` هستند و خروجی آن یک `Promise` است. `option` پارامتر های دلخواهی است که در ارسال درخواست می توانیم در نظر بگیریم. به طور مثال تغییر متد ارسال، `header` و غیره. در صورتی که `option` به تابع پاس داده نشود، متد GET ساده در نظر گرفته می شود. 
...
برای دسترسی به اطلاعات پاسخ این درخواست، یکسری توابع `async` دیگر با خروجی `Promise` وجود دارند. 
